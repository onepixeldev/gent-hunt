<?php
	...
	if(isset($_GET["searchdir"]))
	{
		$search = $_GET["searchdir"];
	}
	...
	if ($pagenum > 1) {
        $previous = $pagenum - 1;
		$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$previous.'&searchdir='.$search.'" class="btn btn-default">Previous</a> &nbsp; &nbsp; ';
 
		for($i = $pagenum-4; $i < $pagenum; $i++){
			if($i > 0){
		        $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$i.'&searchdir='.$search.'" class="btn btn-default">'.$i.'</a> &nbsp; ';
			}
	    }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= ' &nbsp; &nbsp; <a href="'.$_SERVER['PHP_SELF'].'?pn='.$next.'&searchdir='.$search.'" class="btn btn-default">Next</a> ';
    }
	}
